...
def test_MSDA(test_loader, cuda):
    model_root = 'models'
...
def test_CMSS(test_loader, cuda):
    model_root = 'models'

    """ test """
    F_net = torch.load(os.path.join(
        model_root, 'feature_model_CMSS.pth'
    ))
    F_net = F_net.eval()
    C_net = torch.load(os.path.join(
        model_root, 'C_model_CMSS.pth'
    ))
    C_net = C_net.eval()
...
def test_DANN(test_loader, cuda):

    model_root = 'models'

    """ test """
    F_net = torch.load(os.path.join(
        model_root, 'feature_model_DANN.pth'
    ))
    F_net = F_net.eval()
    C_net = torch.load(os.path.join(
        model_root, 'C_model_DANN.pth'
    ))
    C_net = C_net.eval()
...
def test_Sonly(test_loader, cuda):
    model_root = 'models'

    """ test """
    F_net = torch.load(os.path.join(
        model_root, 'feature_model_Sonly.pth'
    ))
    F_net = F_net.eval()
    C_net = torch.load(os.path.join(
        model_root, 'C_model_Sonly.pth'
    ))
    C_net = C_net.eval()
